...
class Document {
 public:
  ...
  size_t GetColumnCount() const {
    const int count =
        static_cast<int>((mData.size() > 0) ? mData.at(0).size() : 0) -
        (mLabelParams.mRowNameIdx + 1);
  ...
  size_t GetRowCount() const {
    const int count =
        static_cast<int>(mData.size()) - (mLabelParams.mColumnNameIdx + 1);
    return (count >= 0) ? static_cast<size_t>(count) : 0;
  }
   ...
   */
  template <typename T>
  T GetCell(const size_t pColumnIdx,
   ...
   */
  template <typename T>
  T GetCell(const std::string& pColumnName,
   ...
   */
  template <typename T>
  void SetCell(const std::string& pColumnName,
               const std::string& pRowName,
               const T& pCell) {
    const int columnIdx = GetColumnIdx(pColumnName);
    if (columnIdx < 0) {
   ...
   */
  template <typename T>
  void SetCell(const size_t pColumnIdx,
               const std::string& pRowName,
               const T& pCell) {
    const int rowIdx = GetRowIdx(pRowName);
    if (rowIdx < 0) {
  ...
  std::vector<std::string> GetColumnNames() const {
    if (mLabelParams.mColumnNameIdx >= 0) {
      return std::vector<std::string>(
          mData.at(static_cast<size_t>(mLabelParams.mColumnNameIdx)).begin() +
              (mLabelParams.mRowNameIdx + 1),
    ...
    if ((mLabelParams.mRowNameIdx >= 0) &&
      ...
      for (auto& dataRow : mData) {
        if (static_cast<int>(dataRow.size()) > mLabelParams.mRowNameIdx) {
          mRowNames[dataRow[static_cast<size_t>(mLabelParams.mRowNameIdx)]] =
              i++;
        }
      }
    }
...
#pragma warning(pop)
  ...
  static void ReplaceString(std::string& pStr,
