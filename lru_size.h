...
struct entry {
  ...
  entry(const entry& pairT) : second(pairT.second), first(pairT.first) {
    bucket = pairT.bucket;
 ...
 public:
  ...
  class iterator {
   public:
    ...
    iterator() {}
    iterator(htype* hash_map, uint32_t bucket)
  ...
  class const_iterator {
   public:
    ...
    const_iterator() {}
    const_iterator(const iterator& proto)
        : _map(proto._map), _bucket(proto._bucket) {}
    const_iterator(const htype* hash_map, uint32_t bucket)
        : _map(hash_map), _bucket(bucket) {}
    ...
    if (std::is_pod<KeyT>::value && std::is_pod<ValueT>::value) {
      ...
      for (uint32_t bucket = 0; bucket < _num_buckets; bucket++) {
        auto next_bucket = NEXT_BUCKET(_pairs, bucket) =
            NEXT_BUCKET(opairs, bucket);
        if (next_bucket != INACTIVE)
          new (_pairs + bucket) PairT(opairs[bucket]);
      }
      NEXT_BUCKET(_pairs, _num_buckets) =
          NEXT_BUCKET(_pairs, _num_buckets + 1) = 0;
    }
  }
  ...
  const_iterator end() const { return {this, _num_buckets}; }
  ...
  constexpr size_type max_bucket_count() const { return (1 << 30); }
  ...
  int get_bucket_info(const uint32_t bucket,
                      uint32_t steps[],
  ...
  void dump_statis() const {
    ...
    for (uint32_t i = 0; i < sizeof(buckets) / sizeof(buckets[0]); i++) {
      ...
      printf("  %2u  %8u  %.2lf  %.2lf\n", i, bucketsi,
             bucketsi * 100.0 * i / _num_filled, sumn * 100.0 / _num_filled);
    }
    ...
    for (uint32_t i = 0; i < sizeof(steps) / sizeof(steps[0]); i++) {
      ...
      if (steps[i] <= 2)
        continue;
      printf("  %2u  %8u  %.2lf  %.2lf\n", i, steps[i],
             steps[i] * 100.0 / collision, sumc * 100.0 / collision);
    }

    if (sumb == 0)
      return;
    printf(
        "    _num_filled/bucket_size/packed collision/cache_miss/hit_find = "
        "%u/%.2lf/%zd/ %.2lf%%/%.2lf%%/%.2lf\n",
        _num_filled, _num_filled * 1.0 / sumb, sizeof(PairT),
        (collision * 100.0 / _num_filled),
        (collision - steps[0]) * 100.0 / _num_filled,
        finds * 1.0 / _num_filled);
  ...
  void clear() {
    if (is_notrivially() || sizeof(PairT) > 32 ||
        _num_filled < _num_buckets / 4)
      clearkv();
    else
  ...
  inline void update_orderid(int32_t incr) { _sum_orderid += incr; }
  ...
  bool remove_half() {
    ...
    //
    //        for (uint32_t src_bucket = 0; src_bucket < _num_buckets;
    //        src_bucket++)
    ...
    snprintf(buff, sizeof(buff),
             "    _num_filled medium_id.pack_size.erase_ids load_factor|%u %u "
             "%zu %u %.3f|, time_use = %d ms",
             _num_filled, medium_id, sizeof(_pairs[0]), erase_ids - _num_filled,
             load_factor(), (int)(clock() - ts));
#if EMHASH_USE_LOG
    static uint32_t iremoves = 0;
    FDLOG("lru_size") << __FUNCTION__ << " removes = " << iremoves++ << "|"
                      << buff << endl;
#else
    puts(buff);
#endif
  ...
  void rehash(uint32_t required_buckets) {
    ...
    if (_num_filled > EMHASH_REHASH_LOG) {
      ...
      snprintf(
          buff, sizeof(buff),
          "    _num_filled/load_factor/K.V/pack/nextid = %u/%.3f/%s.%s/%zd|%u",
          _num_filled, load_factor(), typeid(KeyT).name(),
          typeid(ValueT).name(), sizeof(_pairs[0]),
          entry<KeyT, ValueT>::nextid());
#if EMHASH_USE_LOG
      static uint32_t ihashs = 0;
      FDLOG() << "hash_nums = " << ihashs++ << "|" << __FUNCTION__ << "|"
              << buff << endl;
#else
      puts(buff);
#endif
 ...
 private:
  ...
  uint32_t erase_key(const KeyT& key) {
    ...
    if (next_bucket == bucket) {
      ...
      return next_bucket;
    } /* else if (EMHASH_UNLIKELY(bucket != hash_bucket(EMH_KEY(_pairs,
         bucket)))) return INACTIVE;
     */
    ...
    while (true) {
      ...
      if (_eq(key, EMH_KEY(_pairs, next_bucket))) {
        NEXT_BUCKET(_pairs, prev_bucket) =
  ...
  uint32_t find_filled_bucket(const KeyT& key) {
    const auto bucket = hash_bucket(key);
    auto next_bucket = NEXT_BUCKET(_pairs, bucket);
  ...
  uint32_t find_empty_bucket(const uint32_t bucket_from) {
    ...
    for (uint32_t last = 1, slot = 4;; slot += ++last) {
      ...
      if (NEXT_BUCKET(_pairs, bucket1) == INACTIVE ||
          NEXT_BUCKET(_pairs, ++bucket1) == INACTIVE)
        return bucket1;

      if (last > 4) {
        auto& next = NEXT_BUCKET(_pairs, _num_buckets);
        if (INACTIVE == NEXT_BUCKET(_pairs, next++) ||
            INACTIVE == NEXT_BUCKET(_pairs, next++))
          return next - 1;

        auto medium = (_num_buckets / 2 + next) & _mask;
        if (INACTIVE == NEXT_BUCKET(_pairs, medium) ||
            INACTIVE == NEXT_BUCKET(_pairs, ++medium))
          return medium;
...
// template <class Key, class Val> using emihash = emhash1::lru_cache<Key, Val,
// std::hash<Key>>;
#endif
